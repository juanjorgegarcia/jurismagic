...
for key in repo.vocabulary.keys():
    val = (key, json.dumps(repo.vocabulary[key]["termFrequency"]), repo.vocabulary[key]["docFrequency"], json.dumps(
        repo.vocabulary[key]["postingList"]), repo.vocabulary[key]["term"])
    cur.execute(sql, (val))
